...
class MyResourcePage(BasePage):
    ...
    def get_resource_count(self):
        all_resource_count = int(self.get_ele_text(mloc.all_resource_count, ("运营部-全部资源统计", "all_resource_count")))
        allocated_to_be_process_count = int(self.get_ele_text(mloc.allocated_to_be_process_count,
    ...
    def get_my_depart_count(self):
        all_resource_count = int(self.get_ele_text(mloc.all_resource_count, ("学科部/市场部-全部资源统计", "all_resource_count")))
        allocated_to_be_process_count = int(self.get_ele_text(mloc.allocated_to_be_process_count,
                                                          ("学科部/市场部-新分配待处理统计", "allocated_to_be_process_count")))
        my_following_count = int(self.get_ele_text(mloc.my_following_count, ("学科部/市场部-待跟进统计", "my_following_count")))
